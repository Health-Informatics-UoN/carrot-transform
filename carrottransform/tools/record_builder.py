...
class TargetRecordBuilder(ABC):
    ...
    def __init__(self, context: RecordContext):
        ...
        self.date_col_data = context.omopcdm.get_omop_datetime_linked_fields(context.tgtfilename)
        self.date_component_data = context.omopcdm.get_omop_date_field_components(context.tgtfilename)
        self.notnull_numeric_fields = context.omopcdm.get_omop_notnull_numeric_fields(context.tgtfilename)
    ...
    def create_empty_record(self) -> List[str]:
        ...
        for req_integer in self.notnull_numeric_fields:
    ...
    def apply_original_value_mappings(self, tgtarray: List[str], original_value_fields: List[str], source_value: str):
    ...
    def apply_person_id_mapping(self, tgtarray: List[str]):
        ...
        if (person_id_mapping.dest_field in self.context.tgtcolmap and 
            person_id_mapping.source_field in self.context.srccolmap):
            person_id = self.context.srcdata[self.context.srccolmap[person_id_mapping.source_field]]
    ...
    def apply_date_mappings(self, tgtarray: List[str]) -> bool:
        ...
        source_date = self.context.srcdata[self.context.srccolmap[date_mapping.source_field]]
    ...
    def _apply_single_date_field(self, tgtarray: List[str], dest_field: str, source_date: str) -> bool:
        """Apply a single date field mapping"""
        # Handle date component fields (birth dates with year/month/day)
        if dest_field in self.date_component_data:
            ...
            component_info = self.date_component_data[dest_field]
            if "year" in component_info and component_info["year"] in self.context.tgtcolmap:
                tgtarray[self.context.tgtcolmap[component_info["year"]]] = str(dt.year)
            if "month" in component_info and component_info["month"] in self.context.tgtcolmap:
                tgtarray[self.context.tgtcolmap[component_info["month"]]] = str(dt.month)
            if "day" in component_info and component_info["day"] in self.context.tgtcolmap:
        ...
        elif dest_field in self.date_col_data:
            tgtarray[self.context.tgtcolmap[dest_field]] = source_date
            # Set the linked date-only field
            if self.date_col_data[dest_field] in self.context.tgtcolmap:
                tgtarray[self.context.tgtcolmap[self.date_col_data[dest_field]]] = source_date[:10]
...
class PersonRecordBuilder(TargetRecordBuilder):
    ...
    def build_records(self) -> RecordResult:
        ...
        concept_combinations = generate_combinations(all_concept_mappings) if all_concept_mappings else [{}]
        
        # Create person records for each combination
        records = []
        for concept_combo in concept_combinations:
            record = self._build_single_person_record(concept_combo, all_original_values)
            if record:
    ...
    def _collect_all_mappings(self) -> Tuple[Dict[str, List[int]], Dict[str, str]]:
        ...
        for field_name, concept_mapping in self.context.v2_mapping.concept_mappings.items():
    ...
    def _build_single_person_record(self, concept_combo: Dict[str, int], all_original_values: Dict[str, str]) -> Optional[List[str]]:
...
class StandardRecordBuilder(TargetRecordBuilder):
    ...
    def build_records(self) -> RecordResult:
        ...
        if not valid_value(str(self.context.srcdata[self.context.srccolmap[self.context.srcfield]])):
            self.context.metrics.increment_key_count(
                source=self.context.srcfilename,
                fieldname=self.context.srcfield,
                tablename=self.context.tgtfilename,
        ...
        concept_mapping = self.context.v2_mapping.concept_mappings[self.context.srcfield]
        source_value = str(self.context.srcdata[self.context.srccolmap[self.context.srcfield]])
        ...
        records = []
        for concept_combo in concept_combinations:
            record = self._build_single_standard_record(concept_combo, concept_mapping, source_value)
            if record:
                records.append(record)
            else:
    ...
    def _build_single_standard_record(self, concept_combo: Dict[str, int], concept_mapping: ConceptMapping, source_value: str) -> Optional[List[str]]:
        ...
        # Handle original value fields (direct field copying)
        if concept_mapping.original_value_fields:
            self.apply_original_value_mappings(tgtarray, concept_mapping.original_value_fields, source_value)
