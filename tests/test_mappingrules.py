    ...
    for key in obs_keys:
        ...
        for field, mappings in obs_data[0].items():
            if field == 'person_id':
                ...
                assert set(mappings) == {'observation_concept_id~4135376', 'observation_source_value'}
            elif field == 'ethnicity':
                assert set(mappings) == {'observation_concept_id~44803968', 'observation_source_value'}
...
def test_person_data_correctness(omopcdm, test_rules_file, input_data):
    ...
    assert set(person_mappings['sex']['M']) == {'gender_concept_id~8507', 'gender_source_value'}
    assert set(person_mappings['ethnicity']['white']) == {'race_concept_id~8527'}
    
    # Check second person (Female, Asian)
    assert set(person_mappings['sex']['F']) == {'gender_concept_id~8532', 'gender_source_value'}
    assert set(person_mappings['ethnicity']['asian']) == {'race_concept_id~8515'}
    
    # Check third person (Male, Black)
    assert set(person_mappings['sex']['M']) == {'gender_concept_id~8507', 'gender_source_value'}
    assert set(person_mappings['ethnicity']['black']) == {'race_concept_id~8516', 'race_source_value'}
...
def test_output_data_writing(omopcdm, test_rules_file, input_data):
    ...
    for key in obs_keys:
        ...
        assert len(person_ids) == 1  # Should only have one person_id mapping per observation type
    ...
    for key, data in outdata.items():
        if 'person' in key:
            ...
            total_records += len(input_data)  # One record per person per observation type
    
    expected_records = 1 + (2 * len(input_data))  # 1 person record + 2 observation types per person
