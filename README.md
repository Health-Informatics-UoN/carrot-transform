...
This requires [specifying a connection-string](https://docs.sqlalchemy.org/en/20/tutorial/engine.html) as `--inputs` instead of an input dir folder.
The carrot-mapper workflow should still be used, as if working with .csv files, but carrot-transform can read from an SQLAlchemy database.
...
1. Extract/export some rows from the various tables
    ...
    - something like `SELECT column_name(s) FROM v_head_shapes LIMIT 1000;` is written to `phrenology.csv`
...
3. [when carrot-transform is invoked](https://carrot.ac.uk/transform/quickstart) `--inputs` a database connection string
