...
def test_dock_observations(tmp_path: Path):
    ...
    with DockerImage("carrot_transform", project_root) as image_name:
        ...
        "docker run -it --rm -v.:/run ctra --rules-file /run/mapping.json --output-dir /run/out --person-file /run/demos.csv --input-dir /run/ --omop-ddl-file @carrot/config/OMOPCDM_postgresql_5.3_ddl.sql --omop-config-file @carrot/config/omop.json"
