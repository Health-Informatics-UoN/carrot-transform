...
class InputFileProcessor:
        ...
        try:
            with file_path.open(mode="r", encoding="utf-8-sig") as fh:
                ...
                file_metadata = self._get_file_metadata(
                    source_filename, input_column_map
                )
                ...
                for input_data in csv_reader:
                    ...
                    rejected_id_count += row_result.rejected_id_counts.get(
                        source_filename, 0
                    )
    ...
    def _get_file_metadata(
        self, source_filename: str, input_column_map: Dict[str, int]
    ) -> Optional[Dict[str, Any]]:
        """Get metadata needed for processing this file"""
        datetime_source, person_id_source = (
            self.context.mappingrules.get_infile_date_person_id(source_filename)
        )
        ...
        if datetime_source not in input_column_map:
            logger.warning(
                f"Date field {datetime_source} not found in {source_filename}"
            )
            return None

        return {
            "datetime_col": input_column_map[datetime_source],
            "data_fields": self.context.mappingrules.get_infile_data_fields(
                source_filename
            ),
        }
...
class V2DataProcessor:
    ...
    def process_all_data(self) -> ProcessingResult:
        ...
        for source_filename in self.context.input_files:
            try:
                output_counts, rejected_count = file_processor.process_file(
                    source_filename
                )
